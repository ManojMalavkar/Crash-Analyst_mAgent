...
Usage:
    python evaluation/test_tools.py                    # Run all tests
    python evaluation/test_tools.py --tool search_api  # Test specific tool
    python evaluation/test_tools.py --query "mesh"     # Custom query
"""
...
def test_search_code_examples(query: str = "mesh create example"):
    """Test search_code_examples tool."""
    print("\n" + "="*50)
    print(f"  TEST: search_code_examples(query='{query}')")
    print("="*50)

    try:
        from bin.tool_functions import search_code_examples
        result = search_code_examples(query=query, top_k=3)
...
def main():
    ...
    parser.add_argument(
        "--tool",
        choices=["db", "search_api", "code_examples", "details", "kg", "hierarchy", "all"],
        default="all",
    )
    ...
    if args.tool in ("all", "code_examples"):
        q = args.query or "mesh create example"
        results["search_code_examples"] = test_search_code_examples(q)
